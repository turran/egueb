...
static Eina_Bool _esvg_path_setup(Enesim_Renderer *r, const Esvg_Element_State *estate,
		const Esvg_Shape_Enesim_State *dstate)
{
	...
	//printf("estate->transform = %" ENESIM_MATRIX_FORMAT "\n", ENESIM_MATRIX_ARGS (&estate->transform));
	...
	EINA_LIST_FOREACH(thiz->commands, l, c)
	{
		...
		switch (c->type)
		{
			...
			//printf(" move_to (%f, %f)\n", c->data.move_to.x,  c->data.move_to.y);
