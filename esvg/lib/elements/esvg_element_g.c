 ...
 *                                  Local                                     *
 *============================================================================*/
#define ESVG_LOG_DEFAULT _esvg_element_g_log

static int _esvg_element_g_log = -1;

typedef struct _Esvg_Element_G
{
	/* properties */
	/* private */
...
static Esvg_Element_G * _esvg_element_g_get(Edom_Tag *t)
{
	Esvg_Element_G *thiz;
 ...
 *----------------------------------------------------------------------------*/
static void _esvg_element_g_initialize(Ender_Element *e)
{
	Esvg_Element_G *thiz;
	Edom_Tag *t;

	t = ender_element_object_get(e);
	thiz = _esvg_element_g_get(t);

	thiz->container = esvg_renderable_container_new(e);
}

static Eina_Bool _esvg_element_g_child_add(Edom_Tag *t, Edom_Tag *child)
{
	Esvg_Element_G *thiz;
	Esvg_Type type;
...
static Eina_Bool _esvg_element_g_child_remove(Edom_Tag *t, Edom_Tag *child)
{
	Esvg_Element_G *thiz;
	Esvg_Type type;
...
static Enesim_Renderer * _esvg_element_g_renderer_get(Edom_Tag *t)
{
	Esvg_Element_G *thiz;
...
static Esvg_Element_Setup_Return _esvg_element_g_setup(Edom_Tag *t,
		Esvg_Context *c,
		...
		Enesim_Error **error)
{
	...
	ret = esvg_element_internal_child_setup(t, c, error, NULL, _esvg_element_g_setup_post, thiz);
...
static void _esvg_element_g_free(Edom_Tag *t)
{
	Esvg_Element_G *thiz;
...
static Eina_Bool _esvg_element_g_renderer_propagate(Edom_Tag *t,
		Esvg_Context *c,
...
static Esvg_Renderable_Descriptor _descriptor = {
	/* .child_add		= */ _esvg_element_g_child_add,
	/* .child_remove	= */ _esvg_element_g_child_remove,
	/* .attribute_get 	= */ NULL,
	/* .cdata_set 		= */ NULL,
	/* .text_set 		= */ NULL,
	/* .text_get 		= */ NULL,
	/* .free 		= */ _esvg_element_g_free,
	/* .initialize 		= */ _esvg_element_g_initialize,
	/* .attribute_set 	= */ NULL,
	/* .attribute_animated_fetch = */ NULL,
	/* .setup		= */ _esvg_element_g_setup,
	/* .renderer_get	= */ _esvg_element_g_renderer_get,
	/* .renderer_propagate	= */ _esvg_element_g_renderer_propagate,
};
/*----------------------------------------------------------------------------*
 *                           The Ender interface                              *
 *----------------------------------------------------------------------------*/
static Edom_Tag * _esvg_element_g_new(void)
{
	Esvg_Element_G *thiz;
	Edom_Tag *t;
	Enesim_Renderer *r;

	thiz = calloc(1, sizeof(Esvg_Element_G));
	if (!thiz) return NULL;
 ...
 *                                 Global                                     *
 *============================================================================*/
void esvg_element_g_init(void)
{
	_esvg_element_g_log = eina_log_domain_register("esvg_element_g", ESVG_LOG_COLOR_DEFAULT);
	if (_esvg_element_g_log < 0)
	{
		EINA_LOG_ERR("Can not create log domain.");
		return;
	}
	_esvg_element_g_init();
}

void esvg_element_g_shutdown(void)
{
	if (_esvg_element_g_log < 0)
		return;
	_esvg_element_g_shutdown();
	eina_log_domain_unregister(_esvg_element_g_log);
	_esvg_element_g_log = -1;
}
/*============================================================================*
 *                                   API                                      *
 *============================================================================*/
EAPI Ender_Element * esvg_element_g_new(void)
{
