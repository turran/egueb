 ...
 *                                  Local                                     *
 *============================================================================*/
#define ESVG_LOG_DEFAULT _esvg_element_svg_log

static int _esvg_element_svg_log = -1;

static Ender_Property *ESVG_ELEMENT_SVG_X;
static Ender_Property *ESVG_ELEMENT_SVG_Y;
static Ender_Property *ESVG_ELEMENT_SVG_WIDTH;
static Ender_Property *ESVG_ELEMENT_SVG_HEIGHT;
static Ender_Property *ESVG_ELEMENT_SVG_VIEWBOX;
static Ender_Property *ESVG_ELEMENT_SVG_X_DPI;
static Ender_Property *ESVG_ELEMENT_SVG_Y_DPI;
static Ender_Property *ESVG_ELEMENT_SVG_CONTENT_SCRIPT_TYPE;

static Ender_Property *ESVG_ELEMENT_SVG_ACTUAL_WIDTH;
static Ender_Property *ESVG_ELEMENT_SVG_ACTUAL_HEIGHT;
static Ender_Property *ESVG_ELEMENT_SVG_CONTAINER_WIDTH;
static Ender_Property *ESVG_ELEMENT_SVG_CONTAINER_HEIGHT;

static Ender_Function *ESVG_ELEMENT_SVG_ELEMENT_GET_BY_ID;

typedef struct _Esvg_Element_Svg
{
	/* properties */
	double version;
	Esvg_Rect view_box;
	...
	const Esvg_Element_Svg_Application_Descriptor *application_descriptor;
...
typedef struct _Esvg_Element_Svg_Uri_Data
{
	Esvg_Element_Svg *thiz;
	void *ret;
	void *data;
} Esvg_Element_Svg_Uri_Data;

typedef struct _Esvg_Element_Svg_Uri_Image_Data
{
	Esvg_Element_Svg *thiz;
	Enesim_Surface **s;
	double width;
	double height;
} Esvg_Element_Svg_Uri_Image_Data;

typedef struct _Esvg_Element_Svg_Image
{
	Ender_Element *svg;
	/* TODO the image tag that has a svg file set */
	Edom_Tag *referal;
	Enesim_Surface *s;
} Esvg_Element_Svg_Image;

static Eina_Bool _esvg_element_svg_child_initialize(Edom_Tag *t, Edom_Tag *child_t, void *data);
static Eina_Bool _esvg_element_svg_child_deinitialize(Edom_Tag *t, Edom_Tag *child_t, void *data);

static Esvg_Element_Svg * _esvg_element_svg_get(Edom_Tag *t)
{
	Esvg_Element_Svg *thiz;
...
static const char * _esvg_element_svg_base_dir_get(Esvg_Element_Svg *thiz)
{
...
static Eina_Bool _esvg_element_svg_damage_cb(Enesim_Renderer *r,
...
static Eina_Bool _esvg_element_svg_relative_to_absolute(Esvg_Element_Svg *thiz, const char *relative, char *absolute, size_t len)
{
	const char *base_dir;
	size_t base_dir_len;

	base_dir = _esvg_element_svg_base_dir_get(thiz);
	if (!base_dir)
	{
...
static inline void _esvg_element_svg_size_apply(Esvg_Element_Svg *thiz, Esvg_Element_Context *ctx)
{
	double width;
	double height;
...
static inline void _esvg_element_svg_viewbox_apply(Esvg_Element_Svg *thiz, Esvg_Element_Context *ctx)
{
	Enesim_Matrix scale;
	double width;
	double height;
...
static void _esvg_element_svg_element_changed_add(Esvg_Element_Svg *thiz, Ender_Element *e)
{
...
static void _esvg_element_svg_element_changed_remove(Esvg_Element_Svg *thiz, Ender_Element *e)
{
 ...
 *----------------------------------------------------------------------------*/
static void _esvg_element_svg_resolve_uri_local_get(const char *name,
		const char *fragment, void *user_data)
{
	Esvg_Element_Svg_Uri_Data *data = user_data;
	Esvg_Element_Svg *thiz = data->thiz;
	char **real = data->ret;
...
static void _esvg_element_svg_resolve_uri_absolute_get(const char *name,
		const char *fragment, void *user_data)
{
	Esvg_Element_Svg_Uri_Data *data = user_data;
	Esvg_Element_Svg *thiz = data->thiz;
...
static void _esvg_element_svg_resolve_uri_relative_get(const char *name,
		const char *fragment, void *user_data)
{
	Esvg_Element_Svg_Uri_Data *data = user_data;
	Esvg_Element_Svg *thiz = data->thiz;
	...
	if (!_esvg_element_svg_relative_to_absolute(thiz, name, absolute, PATH_MAX))
		return;
...
static Esvg_Uri_Descriptor _uri_resolve_descriptor = {
	/* .local_get 		= */ _esvg_element_svg_resolve_uri_local_get,
	/* .absolute_get 	= */ _esvg_element_svg_resolve_uri_absolute_get,
	/* .relative_get 	= */ _esvg_element_svg_resolve_uri_relative_get,
};
/*----------------------------------------------------------------------------*
 *                     The URI interface for elements                         *
 *----------------------------------------------------------------------------*/
static void _esvg_element_svg_element_uri_local_get(const char *name,
		const char *fragment, void *user_data)
{
	Esvg_Element_Svg_Uri_Data *data = user_data;
	Esvg_Element_Svg *thiz = data->thiz;
...
static void _esvg_element_svg_element_uri_absolute_get(const char *name,
		const char *fragment, void *user_data)
{
	printf("TODO fetching an absolute uri element '%s'\n", name);
}

static void _esvg_element_svg_element_uri_relative_get(const char *name,
		const char *fragment, void *user_data)
{
	Esvg_Element_Svg_Uri_Data *data = user_data;
	Esvg_Element_Svg *thiz = data->thiz;
...
static Esvg_Uri_Descriptor _uri_element_descriptor = {
	/* .local_get 		= */ _esvg_element_svg_element_uri_local_get,
	/* .absolute_get 	= */ _esvg_element_svg_element_uri_absolute_get,
	/* .relative_get 	= */ _esvg_element_svg_element_uri_relative_get,
};
/*----------------------------------------------------------------------------*
 *                      The URI interface for images                          *
 *----------------------------------------------------------------------------*/
static void _esvg_element_svg_image_uri_local_get(const char *name,
		const char *fragment, void *user_data)
{
	printf("TODO fetching a local image?\n");
}

static void _esvg_element_svg_image_uri_absolute_get(const char *name,
		const char *fragment, void *user_data)
{
	Esvg_Element_Svg_Uri_Image_Data *data = user_data;
	Esvg_Element_Svg *thiz = data->thiz;
...
static void _esvg_element_svg_image_uri_relative_get(const char *name,
		const char *fragment, void *user_data)
{
	Esvg_Element_Svg_Uri_Image_Data *data = user_data;
	Esvg_Element_Svg *thiz = data->thiz;
	...
	_esvg_element_svg_image_uri_absolute_get(absolute, fragment, user_data);
}

static Esvg_Uri_Descriptor _uri_image_descriptor = {
	/* .local_get 		= */ _esvg_element_svg_image_uri_local_get,
	/* .absolute_get 	= */ _esvg_element_svg_image_uri_absolute_get,
	/* .relative_get 	= */ _esvg_element_svg_image_uri_relative_get,
};

static Eina_Bool _esvg_element_svg_child_topmost_set(Edom_Tag *t, Edom_Tag *child,
		void *data)
{
...
static void _esvg_element_svg_child_id_cb(Ender_Element *e, const char *event_name, void *event_data, void *data)
{
	Esvg_Element_Svg *thiz = data;
...
static void _esvg_element_svg_child_mutation_child_cb(Ender_Element *e, const char *event_name, void *event_data, void *data)
{
	Ender_Event_Mutation_Property *ev = event_data;
 ...
 * element
 */
static void _esvg_element_svg_topmost_changed_cb(Ender_Element *e, const char *event_name,
		void *event_data, void *data)
{
	...
	if (ev->previous == e)
	{
		...
		thiz = _esvg_element_svg_get(topmost_p);
		/* remove previous callbacks on the element */
		_esvg_element_svg_child_deinitialize(topmost_p, child_t, thiz);
	}
	if (ev->current == e)
	{
		Esvg_Element_Svg *thiz;
		...
		thiz = _esvg_element_svg_get(topmost_c);
		/* setup all the needed callbacks on the element */
		_esvg_element_svg_child_initialize(topmost_c, child_t, thiz);
	}
}

static void _esvg_element_svg_child_mutation_cb(Ender_Element *e, const char *event_name, void *event_data, void *data)
{
	Esvg_Element_Svg *thiz = data;

	/* add the element to the list of changed elements */
	/* TODO if the property is the child, dont do nothing */
	_esvg_element_svg_element_changed_add(thiz, e);
}
...
static Eina_Bool _esvg_element_svg_child_initialize(Edom_Tag *t, Edom_Tag *child_t, void *data)
{
	...
	child_e = esvg_element_ender_get(child_t);
	ender_event_listener_add(child_e, "Mutation:child", _esvg_element_svg_child_mutation_child_cb, thiz->thiz_e);
	/* add a callback whenever any property has changed to add it the list
	 * of changed elements */
	ender_event_listener_add(child_e, "Mutation", _esvg_element_svg_child_mutation_cb, thiz);
	/* add an event whenever the child changes the id */
	ender_event_listener_add(child_e, "Mutation:id", _esvg_element_svg_child_id_cb, thiz);
	...
	_esvg_element_svg_element_changed_add(thiz, child_e);
	/* set the topmost on every element */
	edom_tag_child_foreach(child_t, _esvg_element_svg_child_topmost_set, thiz->thiz_e);
...
static Eina_Bool _esvg_element_svg_child_deinitialize(Edom_Tag *t, Edom_Tag *child_t, void *data)
{
	 ...
	 * to initialize that child too */
	child_e = esvg_element_ender_get(child_t);
	ender_event_listener_remove_full(child_e, "Mutation:child", _esvg_element_svg_child_mutation_child_cb, thiz->thiz_e);
	/* add a callback whenever any property has changed to add it the list
	 * of changed elements */
	ender_event_listener_remove_full(child_e, "Mutation", _esvg_element_svg_child_mutation_cb, thiz);
	/* add an event whenever the child changes the id */
	ender_event_listener_remove_full(child_e, "Mutation:id", _esvg_element_svg_child_id_cb, thiz);
	/* add the id to the hash of ids */
	esvg_element_id_get(child_e, &id);
	...
	/* mark it for processing */
	_esvg_element_svg_element_changed_remove(thiz, child_e);
	/* set the topmost on every element */
	edom_tag_child_foreach(child_t, _esvg_element_svg_child_topmost_set, NULL);
 ...
 *----------------------------------------------------------------------------*/
static Ender_Element * _esvg_element_svg_element_at(void *data, int x, int y)
{
	Edom_Tag *t = data;
	Esvg_Element_Svg *thiz;
	Eina_Rectangle in;
...
static Esvg_Input_Descriptor _esvg_element_svg_input_descriptor = {
	/* .element_at 	= */ _esvg_element_svg_element_at,
};
/*----------------------------------------------------------------------------*
 *                       The Esvg Renderable interface                        *
 *----------------------------------------------------------------------------*/
static void _esvg_element_svg_initialize(Ender_Element *e)
{
	Esvg_Element_Svg *thiz;
	Edom_Tag *t;
	...
	ender_event_listener_add(e, "TopmostChanged", _esvg_element_svg_topmost_changed_cb, NULL);
	/* own input system */
	thiz->input = esvg_input_new(&_esvg_element_svg_input_descriptor, t);
}

static Eina_Bool _esvg_element_svg_attribute_set(Ender_Element *e, const char *key, const char *value)
{
	if (strcmp(key, "version") == 0)
	{
		Esvg_Number n;
		esvg_number_string_from(&n, value, 0.0);
		esvg_element_svg_version_set(e, n.value);
	}
	...
	else if (strcmp(key, "width") == 0)
	{
		...
		esvg_element_svg_width_set(e, &width);
	}
	else if (strcmp(key, "height") == 0)
	{
		...
		esvg_element_svg_height_set(e, &height);
	}
	else if (strcmp(key, "viewBox") == 0)
	{
		Esvg_Rect viewbox;
		esvg_rect_string_from(&viewbox, value);
		esvg_element_svg_viewbox_set(e, &viewbox);
	}
	else if (strcmp(key, "contentScriptType") == 0)
	{
		esvg_element_svg_content_script_type_set(e, value);
	}
	else
	{
		return EINA_FALSE;
...
static Eina_Bool _esvg_element_svg_attribute_get(Edom_Tag *tag, const char *attribute, char **value)
{
	return EINA_FALSE;
}

static Eina_Bool _esvg_element_svg_child_add(Edom_Tag *t, Edom_Tag *child)
{
	Esvg_Element_Svg *thiz;
	Esvg_Type type;
	Ender_Element *e;
...
static Eina_Bool _esvg_element_svg_child_remove(Edom_Tag *t, Edom_Tag *child)
{
	Esvg_Element_Svg *thiz;
	Esvg_Type type;
...
static Esvg_Element_Setup_Return _esvg_element_svg_setup(Edom_Tag *t,
		Esvg_Context *c,
		...
		Enesim_Error **error)
{
	 ...
	 */
	_esvg_element_svg_element_changed_setup(thiz, c, error);

	return ret;
}

static Enesim_Renderer * _esvg_element_svg_renderer_get(Edom_Tag *t)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_free(Edom_Tag *t)
{
	Esvg_Element_Svg *thiz;
...
static Esvg_Renderable_Descriptor _descriptor = {
	/* .child_add		= */ _esvg_element_svg_child_add,
	/* .child_remove	= */ _esvg_element_svg_child_remove,
	/* .attribute_get 	= */ _esvg_element_svg_attribute_get,
	/* .cdata_set 		= */ NULL,
	/* .text_set 		= */ NULL,
	/* .text_get 		= */ NULL,
	/* .free 		= */ _esvg_element_svg_free,
	/* .initialize 		= */ _esvg_element_svg_initialize,
	/* .attribute_set 	= */ _esvg_element_svg_attribute_set,
	/* .attribute_animated_set = */ NULL,
	/* .setup		= */ _esvg_element_svg_setup,
	/* .renderer_get	= */ _esvg_element_svg_renderer_get,
 ...
 *                           The Ender interface                              *
 *----------------------------------------------------------------------------*/
static Edom_Tag * _esvg_element_svg_new(void)
{
	Esvg_Element_Svg *thiz;
	Edom_Tag *t;
	Enesim_Renderer *r;

	thiz = calloc(1, sizeof(Esvg_Element_Svg));
	if (!thiz) return NULL;
...
static Edom_Tag * _esvg_element_svg_element_find(Edom_Tag *tag, const char *id)
{
	Esvg_Element_Svg *thiz;

	thiz = _esvg_element_svg_get(tag);
	return eina_hash_find(thiz->ids, id);
}

static void _esvg_element_svg_version_set(Edom_Tag *t, double version)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_version_get(Edom_Tag *t, double *version)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_x_set(Edom_Tag *t, Esvg_Coord *x)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_x_get(Edom_Tag *t, Esvg_Coord *x)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_y_set(Edom_Tag *t, Esvg_Coord *y)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_y_get(Edom_Tag *t, Esvg_Coord *y)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_width_set(Edom_Tag *t, Esvg_Length *width)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_width_get(Edom_Tag *t, Esvg_Length *width)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_height_set(Edom_Tag *t, Esvg_Length *height)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_height_get(Edom_Tag *t, Esvg_Length *height)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_viewbox_set(Edom_Tag *t, Esvg_Rect *vb)
{
	Esvg_Element_Svg *thiz;
 ...
 * the preferred area
 */
static void _esvg_element_svg_actual_width_get(Edom_Tag *t, double *actual_width)
{
	Esvg_Element_Svg *thiz;
	double aw;
	double cw;
	...
	thiz = _esvg_element_svg_get(t);
	esvg_element_svg_internal_container_width_get(t, &cw);
	aw = esvg_coord_final_get(&thiz->width, cw, thiz->base_font_size);
	*actual_width = aw;
}

static void _esvg_element_svg_actual_height_get(Edom_Tag *t, double *actual_height)
{
	Esvg_Element_Svg *thiz;
	double ah;
	double ch;
	...
	thiz = _esvg_element_svg_get(t);
	esvg_element_svg_internal_container_height_get(t, &ch);
	ah = esvg_coord_final_get(&thiz->height, ch, thiz->base_font_size);
	*actual_height = ah;
}

static void _esvg_element_svg_container_width_set(Edom_Tag *t, double container_width)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_container_height_set(Edom_Tag *t, double container_height)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_x_dpi_set(Edom_Tag *t, double x_dpi)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_x_dpi_get(Edom_Tag *t, double *x_dpi)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_y_dpi_set(Edom_Tag *t, double y_dpi)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_y_dpi_get(Edom_Tag *t, double *y_dpi)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_content_script_type_set(Edom_Tag *t, const char *v)
{
	Esvg_Element_Svg *thiz;
...
static void _esvg_element_svg_content_script_type_get(Edom_Tag *t, const char **v)
{
	Esvg_Element_Svg *thiz;
...
static Ender_Element *_esvg_element_svg_element_get_by_id(Edom_Tag *t, const char *id)
{
	Esvg_Element_Svg *thiz;
	Ender_Element *found;
...
/* The ender wrapper */
#define _esvg_element_svg_delete NULL
#define _esvg_element_svg_x_is_set NULL
#define _esvg_element_svg_y_is_set NULL
#define _esvg_element_svg_width_is_set NULL
#define _esvg_element_svg_height_is_set NULL
#define _esvg_element_svg_actual_width_set NULL
#define _esvg_element_svg_actual_width_is_set NULL
#define _esvg_element_svg_actual_height_set NULL
#define _esvg_element_svg_actual_height_is_set NULL
#define _esvg_element_svg_viewbox_get NULL
#define _esvg_element_svg_viewbox_is_set NULL
#define _esvg_element_svg_container_width_get esvg_element_svg_internal_container_width_get
#define _esvg_element_svg_container_width_is_set NULL
#define _esvg_element_svg_container_height_get esvg_element_svg_internal_container_height_get
#define _esvg_element_svg_container_height_is_set NULL
...
{
	_esvg_element_svg_log = eina_log_domain_register("esvg_element_svg", ESVG_LOG_COLOR_DEFAULT);
	if (_esvg_element_svg_log < 0)
	{
		EINA_LOG_ERR("Can not create log domain.");
		return;
	}
	/* FIXME: initialize the ender interface */
	_esvg_element_svg_init();
}
...
void esvg_element_svg_shutdown(void)
{
	if (_esvg_element_svg_log < 0)
		return;
	_esvg_element_svg_shutdown();
	eina_log_domain_unregister(_esvg_element_svg_log);
	_esvg_element_svg_log = -1;
}
/*----------------------------------------------------------------------------*
 ...
 *----------------------------------------------------------------------------*/
Esvg_Video_Provider_Descriptor * esvg_element_svg_video_provider_descriptor_get(
		Ender_Element *e)
{
	Esvg_Element_Svg *thiz;
	const Esvg_Element_Svg_Application_Descriptor *descriptor;
...
{
	Esvg_Element_Svg *thiz;
	Esvg_Element_Svg_Uri_Data data;
	Edom_Tag *t;
...
{
	Esvg_Element_Svg *thiz;
	Esvg_Element_Svg_Uri_Image_Data data;
	Edom_Tag *t;
...
#if 0
Esvg_Element_Svg_External * esvg_element_svg_external_new(Ender_Element *svg, Edom_Tag *t, Enesim_Surface *s)
{
	Esvg_Element_Svg_External *external;

	external = calloc(1, sizeof(Esvg_Element_Svg_External));
	external->svg = svg;
	external->owner = t;
	external->s = s;
}

void esvg_element_svg_external_add(Ender_Element *e, Esvg_Element_Svg_External *external)
{
	Edom_Tag *t;
...
void esvg_element_svg_external_remove(Ender_Element *e, Esvg_Element_Svg_External *external)
{
	thiz->externals = eina_list_remove(thiz->externals, external);
}

void esvg_element_svg_external_free(Esvg_Element_Svg_External *external)
{
	free(external);
}
#endif
...
Ender_Element * esvg_element_svg_svg_load(Ender_Element *e, const char *uri)
{
	...
	final = esvg_element_svg_uri_resolve(e, uri);
...
#if 0
void esvg_element_svg_style_add(Edom_Tag *tag, Esvg_Parser_Style *s)
{
	Esvg_Element_Svg *thiz;
...
void esvg_element_svg_style_apply(Edom_Tag *tag)
{
	Esvg_Element_Svg *thiz;
	Esvg_Parser_Style *s;
	Eina_List *l;
...
EAPI Ender_Element * esvg_element_svg_element_find(Ender_Element *e, const char *id)
{
	...
	t = ender_element_object_get(e);
	return _esvg_element_svg_element_get_by_id(t, id);
}
...
EAPI void esvg_element_svg_width_get(Ender_Element *e, Esvg_Length *width)
{
	...
	t = ender_element_object_get(e);
	_esvg_element_svg_width_get(t, width);
}
...
EAPI void esvg_element_svg_height_get(Ender_Element *e, Esvg_Length *height)
{
	...
	t = ender_element_object_get(e);
	_esvg_element_svg_height_get(t, height);
}
...
EAPI void esvg_element_svg_content_script_type_get(Ender_Element *e, const char **v)
{
	Edom_Tag *t;
	t = ender_element_object_get(e);
	_esvg_element_svg_content_script_type_get(t, v);
}
...
EAPI void esvg_element_svg_actual_width_get(Ender_Element *e, double *actual_width)
{
	...
	t = ender_element_object_get(e);
	_esvg_element_svg_actual_width_get(t, actual_width);
}
...
EAPI void esvg_element_svg_actual_height_get(Ender_Element *e, double *actual_height)
{
	...
	t = ender_element_object_get(e);
	_esvg_element_svg_actual_height_get(t, actual_height);
}
...
EAPI void esvg_element_svg_damages_get(Ender_Element *e, Esvg_Element_Svg_Damage_Cb cb, void *data)
{
	...
	enesim_renderer_damages_get(r, _esvg_element_svg_damage_cb, thiz->tiler);
...
EAPI const char * esvg_element_svg_base_dir_get(Ender_Element *e)
{
	...
	t = ender_element_object_get(e);
	thiz = _esvg_element_svg_get(t);
	return _esvg_element_svg_base_dir_get(thiz);
}
