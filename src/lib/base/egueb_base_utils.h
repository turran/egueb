...
#ifndef _EGUEB_BASE_UTILS_H_
#define _EGUEB_BASE_UTILS_H_

/* FIXME all this generic parsers should have a common prefix
 * and a common attribute name
 */

#define EGUEB_BASE_IS_HEXA(c) \
	(((c >= 48) && (c <= 57)) || \
	((c >= 65) && (c <= 70)) || \
	((c >= 97) && (c <= 102)))

#define EGUEB_BASE_SPACE_SKIP(t) \
	while (*t) { if ((*t == ' ') || (*t == '\n') || (*t == '\t') || (*t == '\r')) t++; else break; }

#define EGUEB_BASE_SPACE_COMMA_SKIP(t) \
	EGUEB_BASE_SPACE_SKIP(t); \
	if (*t == ',') t++; \
	EGUEB_BASE_SPACE_SKIP(t);
