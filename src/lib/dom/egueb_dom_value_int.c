...
static char * _int_string_to(const Egueb_Dom_Value *v)
{
	...
	return eina_extra_str_dup_printf("%d", v->data.i32);
}
...
EAPI void egueb_dom_value_int_set(Egueb_Dom_Value *v, int i)
{
	v->data.i32 = i;
}

EAPI int egueb_dom_value_int_get(Egueb_Dom_Value *v)
{
	return v->data.i32;
}

EAPI void egueb_dom_value_int_data_from(Egueb_Dom_Value *v, Egueb_Dom_Value_Data *data)
{
	v->data.i32 = data->i32;
}

EAPI void egueb_dom_value_int_data_to(Egueb_Dom_Value *v, Egueb_Dom_Value_Data *data)
{
	int *ptr = data->ptr;
	if (!ptr) return;
	*ptr = v->data.i32;
