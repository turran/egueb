 ...
 *                                  Local                                     *
 *============================================================================*/
#define EGUEB_SVG_ATTR_XLINK_HREF_DESCRIPTOR 					\
		egueb_svg_attr_xlink_href_descriptor_get()
#define EGUEB_SVG_ATTR_XLINK_HREF(o) ENESIM_OBJECT_INSTANCE_CHECK(o,		\
		Egueb_Svg_Attr_Xlink_Href, EGUEB_SVG_ATTR_XLINK_HREF_DESCRIPTOR)

typedef struct _Egueb_Svg_Attr_Xlink_Href
{
	...
	Egueb_Dom_Node *node;
} Egueb_Svg_Attr_Xlink_Href;

typedef struct _Egueb_Svg_Attr_Xlink_Href_Class
{
	Egueb_Dom_Attr_Object_Class base;
} Egueb_Svg_Attr_Xlink_Href_Class;
 ...
 *----------------------------------------------------------------------------*/
static Eina_Bool _egueb_svg_attr_xlink_href_value_get(Egueb_Dom_Attr *p,
		Egueb_Dom_Attr_Type type, void ***o)
{
	Egueb_Svg_Attr_Xlink_Href *thiz;

	thiz = EGUEB_SVG_ATTR_XLINK_HREF(p);
	switch (type)
	{
 ...
 *                               Attr interface                               *
 *----------------------------------------------------------------------------*/
static void _egueb_svg_attr_xlink_href_clone(Egueb_Dom_Attr *a, Eina_Bool live,
...
_egueb_svg_attr_xlink_href_value_descriptor_get(Egueb_Dom_Attr *p)
{
...
ENESIM_OBJECT_INSTANCE_BOILERPLATE(EGUEB_DOM_ATTR_OBJECT_DESCRIPTOR,
		Egueb_Svg_Attr_Xlink_Href, Egueb_Svg_Attr_Xlink_Href_Class,
		egueb_svg_attr_xlink_href)

static void _egueb_svg_attr_xlink_href_class_init(void *k)
{
	Egueb_Dom_Attr_Class *p_klass;
	Egueb_Dom_Attr_Object_Class *o_klass;

	p_klass = EGUEB_DOM_ATTR_CLASS(k);
	p_klass->value_descriptor_get = _egueb_svg_attr_xlink_href_value_descriptor_get;
	p_klass->clone = _egueb_svg_attr_xlink_href_clone;

	o_klass = EGUEB_DOM_ATTR_OBJECT_CLASS(k);
	o_klass->value_get = _egueb_svg_attr_xlink_href_value_get;
}

static void _egueb_svg_attr_xlink_href_instance_init(void *o)
{
}

static void _egueb_svg_attr_xlink_href_instance_deinit(void *o)
{
	Egueb_Svg_Attr_Xlink_Href *thiz;

	thiz = EGUEB_SVG_ATTR_XLINK_HREF(o);
	if (thiz->def)
	...
	_egueb_svg_attr_xlink_href_cleanup(thiz);
}
/*============================================================================*
 *                                 Global                                     *
 *============================================================================*/
...
EAPI Eina_Bool egueb_svg_attr_xlink_href_resolve(Egueb_Dom_Node *attr)
{
	...
	thiz = EGUEB_SVG_ATTR_XLINK_HREF(attr);
	...
	if (!egueb_dom_string_is_equal(str, thiz->last))
	{
		...
		if (str)
		{
			...
			if (!egueb_dom_node_document_get(a->owner, &doc, NULL))
			{
				WARN("No document set");
				ret = EINA_FALSE;
				goto no_doc;
			}
			egueb_svg_document_element_get_by_iri(doc, str, &thiz->node);
			if (thiz->node)
			{
...
EAPI void egueb_svg_attr_xlink_href_node_get(Egueb_Dom_Node *attr, Egueb_Dom_Node **n)
{
	...
	thiz = EGUEB_SVG_ATTR_XLINK_HREF(attr);
	egueb_svg_attr_xlink_href_resolve(attr);
	if (thiz->node)
